            ...
            <xsl:for-each-group select="./descendant::*:marker" group-by="./*:initiale"> <!-- trie sur les initiales -->
              ...
              <fieldset id="{@role}{./*:initiale}">
                ...
                <div class="developper">
                  <!-- <xsl:choose>
                    <xsl:when test="./*:nom"> -->
                  <xsl:for-each-group select="current-group()" group-by="./*:value"> <!-- trie en fonction de l'auteur -->
                    <xsl:sort select="./*:nom" order="ascending"/>
                    <p class="entries">
                      <xsl:variable name="identifiant">
                        <xsl:value-of select="substring-after(substring-before(./*:iden, '.xml'), 'p.')"/>
                        <!-- <xsl:value-of select="substring-after(substring-before(./*:iden, '.xml'),'/personnes/')"/> -->
                      </xsl:variable>
                      <a class="personne">
                      ...
                      <xsl:for-each-group select="current-group()" group-by="@id"> <!-- renvoie les fiches correspondantes -->
                        ...
                        <a class="fiche">
                          <xsl:attribute name="href">
                            <xsl:value-of select="concat(replace(./*:baseuri[1], 'xml', 'html'), '#', $identifiant)"/>
                          </xsl:attribute>
